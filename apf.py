...
class Final():
	...
	def SafeDynamicPipeline(self,blockno):
		...
		while (np.linalg.norm(tmpFK[:3,3]-StackPos[:3,3]) > 0.5):
			tmp = self.ik.inverse(StackPos, tmp)
			tmpFK = self.fk.forward(tmp)[1]
		# tmp = self.ik.inverse(DropPos[0],np.array([-0.21878903,  0.42867394, -0.19236141 ,-1.55804698 , 0.08666383 , 1.97835567,0.35685793]))
		#np.vstack([self.get_path(self.eepos, StackPos[0], 20), tmp])
	...
	def StaticBlocksFinal(self):		
		...
		for blockno in self.StaticBlocksIhave:
			...
			self.TargetPos,success = self.alignTarget(self.TargetPos)
			print(self.TargetPos)
			self.TargetPos[2,-1] = 0.235
			#Static Cube Controller
			initial =  self.netq
			targetpos =	 self.ik.inverse(self.TargetPos, initial)
			# q_path =	self.POT.plan(self.map_struct, deepcopy(initial), deepcopy(targetpos))
			from lib.astar import Astar
			import csv
			q_path = Astar(self.map_struct,initial,targetpos)
			# q_path = q_path[::20,:]
			for i in range(len(q_path[:,0])):
				self.arm.safe_move_to_position(q_path[i,:])
			

			print("Iske baad")
			self.updateAllStates()
			self.TargetPos[2,-1] = 0.225
			tmp = self.ik.inverse(self.TargetPos, self.currentangle)
			# self.arm.safe_move_to_position(tmp)
			# self.arm.exec_gripper_cmd(0.049,50)
			print(tmp)
			with open("new_file.csv","w+") as my_csv:
				csvWriter = csv.writer(my_csv,delimiter=',')
				csvWriter.writerows(q_path)
			...
			self.updateAllStates()
			tmp = self.ik.inverse(StackPos, tmp)			
			# q_path = Astar(self.map_struct,self.currentangle,tmp)
			q_path =	self.POT.plan(self.map_struct, deepcopy(self.currentangle), deepcopy(tmp))
			q_path = q_path[::20,:]
			for i in range(len(q_path[:,0])):
				self.arm.safe_move_to_position(q_path[i,:])
